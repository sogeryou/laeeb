...
const auditOptions = ['全部', '待审核', '资料复审', '已认证', '已拒绝', '已移除'];
...
export function CompanionManagementPanel() {
  ...
  return (
    ...
    >
      <div className="mb-4 grid gap-3 rounded-md bg-slate-50 p-3 lg:grid-cols-[1fr_1fr_auto]">
        <Field label="陪玩ID">
      ...
      {detail && (
        <CompanionDetailModal
          ...
          onRemove={() => {
            dispatch({ type: 'COMPANION_REVIEW', payload: { id: detail.id, action: '移除陪玩' } });
            toast(`已移除陪玩 ${detail.name}`, 'error');
            setDetail(null);
          }}
        />
      )}
...
export function CompanionAuditPanel() {
  ...
  const q = useTableQuery<CompanionService>(state.companionServices, {
    extra: (row) => {
      const idNeedle = draftId.trim().toLowerCase();
      if (idNeedle && !row.id.toLowerCase().includes(idNeedle)) return false;
      if (!matchesService(row, serviceType)) return false;
      return auditStatus === '全部' || row.audit === auditStatus;
    },
  });
  const serviceOptions = useMemo(() => allServiceTypes(state.companionServices), [state.companionServices]);
  const selectedVisibleIds = selectedIds.filter((id) => q.filtered.some((row) => row.id === id));

  const toggleSelected = (id: string) => {
    setSelectedIds((prev) => prev.includes(id) ? prev.filter((item) => item !== id) : [...prev, id]);
  };

  const applyReview = (ids: string[], action: '审核通过' | '审核驳回') => {
    ids.forEach((id) => dispatch({ type: 'COMPANION_REVIEW', payload: { id, action } }));
    toast(`已${action} ${ids.length} 个陪玩`, action === '审核驳回' ? 'error' : 'success');
  ...
  return (
    <Panel title="陪玩审核" icon={Sparkles}>
      ...
      <DataTable
        columns={['选择', '陪玩ID', '陪玩昵称', '审核状态']}
        rows={q.pageItems.map((row) => [
          <input type="checkbox" checked={selectedIds.includes(row.id)} onChange={() => toggleSelected(row.id)} className="size-4 accent-emerald-700" />,
          <button type="button" onClick={() => setDetail(row)} className="font-black text-emerald-700 hover:underline">{row.id}</button>,
          row.name,
      ...
      {detail && (
        <CompanionDetailModal
          ...
          mode="audit"
          onClose={() => setDetail(null)}
          onApprove={() => setConfirm({ ids: [detail.id], action: '审核通过' })}
          onReject={() => setConfirm({ ids: [detail.id], action: '审核驳回' })}
        />
      )}
      {confirm && (
        <ConfirmReviewModal
...
function CompanionDetailModal({
  ...
  return (
    <ModalShell title={mode === 'manage' ? '陪玩资料' : '陪玩审核资料'} subtitle={`${row.id} / ${row.name}`} onClose={onClose} maxWidth="max-w-3xl">
      ...
      <section>
        <h4 className="mb-2 text-sm font-black text-slate-700">{mode === 'manage' ? '已通过审核的服务项目' : '提交的服务项目'}</h4>
